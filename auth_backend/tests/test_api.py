...
#         #self.assertTrue('test@gmail.com' in response
#         #self.assertTrue('this cis home' in response.get_data(as_text=True))
#         print(response.get_data())
    ...
    def test_empty_email_registration_fail(self):
        '''
        Test bad email registration
        '''
        # register a new user with incorrect email specification
        response = self.client.post('/register',
            # set the content type header
            content_type = 'application/json',
            # set the data to be sent with the request
            data = json.dumps({
                # email is incorrect as it can't be empty, so the request
    ...
    def test_bad_email_authentication(self):
        '''
        Test wrong email correct password login attmpt
        '''
        # register a new user and test if he is authenticated
        response_register = self.client.post('/register',
            content_type = 'application/json',
        ...
        # test the login endpoint with wrong email
        response_login = self.client.post('/login',
            content_type = 'application/json',
            data = json.dumps({
        ...
        # The login attempt should fail and response status code should be 401
        self.assertEqual(response_login.status_code, 401)
    ...
    def test_validate_token(self):
        ...
        validate_response = self.client.get('/validate_token',
            content_type = 'application/json',
            headers = {
    ...
    def test_validate_fresh_token(self):
        ...
        fresh_validation_response = self.client.get('/validate_fresh_token',
            content_type = 'application/json',
            headers = {
                'Set-Cookie': access_token_cookie,
                'X-CSRF-TOKEN': csrf_access_token.split('=')[1].split(';')[0]
